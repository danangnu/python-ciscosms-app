...
def is_device_register(ip):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    username, password = get_ssh_credentials()
    ssh.connect(ip, username=username, password=password, look_for_keys=False, allow_agent=False)

    stdin, stdout, stderr = ssh.exec_command("show running-config | include logging")
    logging_list = stdout.read().decode()
    ssh.close()

    lines = logging_list.splitlines()
    hosts = [line.replace("logging host ", "").strip() 
            for line in lines if line.strip().startswith("logging host")]

    ip_to_check = get_ip_address()
    if ip_to_check in hosts:
        return True
    else:
        return False
...
def load_devices_from_db():
    ...
    for row in rows:
        ...
        register = is_device_register(ip)
        devices.append({
            "name": row[0],
            "ip": ip,
            "gateway": row[2],
            ...
            "lastSMS": "",
            "signal": 0,
            "status": "Not Register" if register == False else "Online" if online else "Offline",
            "id": row[6]
        })
    return devices
